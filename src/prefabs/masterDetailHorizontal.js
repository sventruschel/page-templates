(() => ({
  name: 'Master Detail View - Horizontal',
  icon: 'DataTable',
  description: 'Datatable with detailview',
  category: 'LAYOUT',
  beforeCreate: ({
    components: {
      Content,
    ...
    return (
      <>
        <Header title="Configure Data Table & Detail View" onClose={close} />
        <Content>
          <Field
            label="Model"
            error={
        ...
        <Footer
          onSave={() => {
            if (!modelId || details.length < 1 || properties.length < 1) {
              setModelValidation(!modelId);
              setDetailsValidation(details.length < 1);
            ...
            if (modelId) {
              ...
              const detailRow = reduceStructure('#detailRow', prefabStructure);
